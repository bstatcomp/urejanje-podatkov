...
\NormalTok{tib }\OtherTok{\textless{}{-}} \FunctionTok{tibble}\NormalTok{(}
  \AttributeTok{ime             =} \FunctionTok{c}\NormalTok{(}\StringTok{"Maja"}\NormalTok{, }\StringTok{"Ales"}\NormalTok{, }\StringTok{"Tom"}\NormalTok{, }\StringTok{"Barbara"}\NormalTok{, }\StringTok{"Simon"}\NormalTok{, }\StringTok{"Tina"}\NormalTok{),}
  \AttributeTok{spol            =} \FunctionTok{c}\NormalTok{(}\StringTok{"z"}\NormalTok{, }\StringTok{"m"}\NormalTok{, }\StringTok{"m"}\NormalTok{, }\StringTok{"z"}\NormalTok{, }\StringTok{"m"}\NormalTok{, }\StringTok{"z"}\NormalTok{),}
  \AttributeTok{ocena           =} \FunctionTok{c}\NormalTok{(}\DecValTok{7}\NormalTok{, }\DecValTok{10}\NormalTok{, }\DecValTok{6}\NormalTok{, }\DecValTok{8}\NormalTok{, }\DecValTok{8}\NormalTok{, }\DecValTok{7}\NormalTok{),}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tib }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{mutate}\NormalTok{(}\AttributeTok{spol =} \FunctionTok{factor}\NormalTok{(spol),}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{write\_csv2}\NormalTok{(ocene, }\StringTok{"./data{-}raw/ocene\_copy.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## Error in write_csv2(ocene, "./data-raw/ocene_copy.csv"): object 'ocene' not found
\end{verbatim}

\textbf{Naloga 1}: Datoteko ocene.csv preberite z funkcijami paketa \textbf{readr} (\texttt{read\_csv2()} in \texttt{write\_csv2()}). Shranite jo kot ocene2.csv in to datoteko preberite nazaj. Nato narediti enako z baznimi funkcijami v R ((\texttt{read.csv2()} in \texttt{write.csv2()})). Kaj opazite?
...
\textbf{Naloga 2}: Ponovno preberite ocene.csv ter:
...
Zbirka paketov tidyverse vsebuje paket \textbf{stringr}, ki je namenjen delu z nizi. Vsi ukazi v paketu se začnejo s \texttt{str\_}, kar v kombinaciji z auto complete funkcionalnostjo omogoča hitro izbiro in pregled vseh funkcij.
...
Niz lahko vključuje tudi vse posebne znake, ki jih podpira unicode. Za vnos unicode znakov uporabite \textbf{\textbackslash uxxxx}, kjer je \textbf{xxxx} koda znaka, ki ga lahko poiščete na strani \url{https://unicode.org/charts/}.
...
Velikokrat se pri delu z nizi srečamo tudi s tem, da potrebujemo le del niza ali pa niz razdeliti na manjše. Poglejmo si na primeru niza \texttt{"\ \ \ \ KUPI:\ paradižnik,\ bučke,\ jabolka,\ paprika,\ jagode.\ \ "} iz katerega želimo dobiti vektor sadja in zelenjave.
...
Najprej odstranimo odvečne začetne in končne presledke. To storimo z \texttt{str\_trim()}.
...
Z uporabo funkcije \texttt{str\_sub()} nam vrne podniz glede na indekse znakov.
...
Sedaj, ko imamo elemente samo naštete in ločene z vejico jih lahko preprosto razdelimo z funkcijo \texttt{str\_split()}, ki mu podamo vzorec za ločevanje.
...
Uporabimo najprej funkcijo \texttt{str\_detect()}, ki vrne TRUE, če niz vsebuje vzorec, drugače pa FALSE.
...
Z funkcijo \texttt{str\_locate()} lahko vrnemo indekse lokacij najdenih vzorcev, z \texttt{str\_extract()} pa lahko dobimo ta vzorec oziroma podniz. Z \texttt{str\_match()} pa lahko celo dobimo podskupine.
...
Preprosta in zelo uporaba pa je funkcija \texttt{str\_replace()} oziroma \texttt{str\_replace\_all} s katero lahko zamenjamo dele nizov.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{flights }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{select}\NormalTok{(year, month, day, hour, minute) }\SpecialCharTok{\%\textgreater{}\%}
