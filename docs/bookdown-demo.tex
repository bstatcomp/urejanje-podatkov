...
Spremenimo \textbf{spol} v nominalno spremenljivko in \textbf{oceno} v ordinalno spremenljivko.
...
\begin{verbatim}
## Error in make_date(year = 2021, month = 6, day = 11): could not find function "make_date"
\end{verbatim}
...
\begin{verbatim}
## Error in make_datetime(year = 2021, month = 6, day = 11, hour = 11, min = 30, : could not find function "make_datetime"
\end{verbatim}

\hypertarget{branje-in-pisanje-podatkov}{%
\subsubsection{Branje in pisanje podatkov}\label{branje-in-pisanje-podatkov}}

Datoteke s končnico csv smo na tej delavnici že brali s funkcijo \texttt{csv.read2()}. V paketu \textbf{readr} obstaja sorodna funkcija \texttt{csv\_read()}.

Preberimo si sedaj datoteko \texttt{ocene.csv}, kjer so prejšnji tabeli dodani še nekateri stolpci. Paket nam tudi pove kašnega tipa so stolpci.
...
Podobno lahko zapišemo podatke nazaj na disk z metodo \texttt{write\_csv2()}.
...
\tightlist
\item
  spremenite stolpec opravljene\_vaje v kategorične vrednosti
\item
  zamenjajte stolpce leto\_izpita, mesec\_izpita, dan\_izpita z datum\_izpita, ki vsebuje vse te podatke
\item
...
\begin{verbatim}
## Error: Problem with `mutate()` column `datum_izpita`.
## i `datum_izpita = make_date(year = leto_izpita, month = mesec_izpita, day = dan_izpita)`.
## x could not find function "make_date"
\end{verbatim}
...
Oziroma za slovenske Windowse: ``Nastavitve -\textgreater{} Ura in Jezik -\textgreater{} Regija -\textgreater{} Območne nastavitve'' in izberite ``Slovenščina (Slovenija)''
