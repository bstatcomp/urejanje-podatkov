...
Ko imamo v bazi svoje podatke, lahko po njih delamo SQL poizvedbe. Osnovna funkcija je `dbGetQuery()`, ki zahteva povezavo in poizvedbo zapisano kot navaden tekst.
...
Vidimo, da nam poizvedba vrne pričakovani rezultat. Če želimo prenesti celotno tabelo imamo na voljo funkcijo `dbReadTable()`. Z uporabo le te preberimo podatke o transakcijah, ki smo jih za potrebe demonstracije že predhodno naložili. Podatki so enako kot na drugem predavanju.
...
Tudi paket `dplyr` ima funkcije za delo z bazami, ki vedno vrnejo tibble, hkrati pa nam omogočajo **pisati** SQL ukaze z pipe operacijo.

Dostop do tabele na bazi lahko omogočimo s funkcijo `tbl`. Ta funkcija podatkov ne prenese ampak naredi le kazalec na tabelo na bazi!
...
Sedaj naredimo kratko poizvedbo, kot smo jih navajeni delati z *tidyverse* paketi.
...
Funkcija `show_query()` nam pokaže SQL stavek, ki se je izvedel direktno na bazi, da dobimo želeni rezultat. To pomeni, da se je zgornji izraz le prevedel v SQL poizvedbo in bil poslan bazi.
...
Paket `odbc` omogoča povezavo do različnih baz, hkrati pa je tudi povezan z RStudijem, kar lahko vidite v desnem zgornjem oknu pod zavihkom connections.
...
Najbolj osnovna in najbolj uporabna funkcija za vzorčenje tabel v R-ju je `sample`. Omogoča vračanje elementov s ali brez ponavljanja in tudi uporabo uteži.
...
Takšno vzorčenje je v praksi reprezantitivno, če imamo dovolj podatkov in imamo dokaj enakomerno število vrednosti pri vsaki spremenljivki.
...
Opazimo, lahko da je ujemanje OK, vendar ne idealno. Pogledamo lahko tudi, če smo pravilno vzorčili zvezno spremenljivko, pri tem pa jo bomo za lažji prikaz diskretizirali in prikazali obe distribuciji.
...
Najprej za spremenljivko *type* izračunajmo koliko primerov bi moralo biti izbranih za vsako vrednost, če bi želeli imeti enako distribucijo kot v originalnih podatkih.
...
Spodaj je še funkcija, ki nam vrne isto, hkrati pa pazi še za morebitne napake pri zaokroževanju.
...
V tem primeru je bilo to še dokaj preprosto, da smo brez večjih težav lahko to spisali samo. Vendar, ta pristop ohranja le distribucijo po izbrani spremenljivi, ostale pa lahko tudi nekoliko bolj pokvari.
