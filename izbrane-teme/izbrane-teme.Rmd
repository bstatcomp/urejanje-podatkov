...
* **Podatke**.
* **Tip grafa** -- Kako naj bodo podatki predstavljeni na grafu, lahko jih podatmo tudi več.
* **Estetike** -- x-os, y-os, barve točk, skupine podatkov itd...

Prva dva elementa sta nujno potreba. Estetike pa lahko izpustimo, pri čemer jih bo `ggplot()` samodejno smiselno določil.

Poglejmo si različne grafe osnovne grafe na podatkih portugalsih požarov.
...
```
Zgornje podatke bi lahko izrisali tudi v obliki stolpičnega grafa s funkcijo `geom_bar()`, ki pa že implicitno prešteje število vrstic za vsako vrednost x-a.
...
Izrišimo sedaj graf z pisavo 'Ravie':
...
Predlagamo, da poizkusite tudi sami, ampak z več podatki in si izpišete razliko v časih. Pri tem najprej preverite, koliko jeder ima vaš računalnik, potrebovali boste vsaj 2. Glede na to, da imamo samo 2 razpredelnici, več kot 2 jeder ni smiselno uporabiti.
