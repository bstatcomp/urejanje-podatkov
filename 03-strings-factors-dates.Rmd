...
```
  ...
  d) Ugotovite koliko časovnih pasov je preletel vsak let. Rezultat izpišite v `tz_diff`.
  
    ```{r, echo = FALSE, message = FALSE}
#uporabimo tabelo podatki iz c) dela
podatki <- left_join(flights, airports, by = c("origin" = "faa")) %>%
      mutate(tz_origin = tz) %>%
      select(year, month, day, sched_dep_time, 
         origin, dest, tz_origin)
podatki <- left_join(podatki, airports, by = c("dest" = "faa")) %>%
  mutate(tz_dest = tz) %>%
  select(year, month, day, sched_dep_time, 
         origin, dest, tz_origin, tz_dest) %>%
  mutate(tz_diff = abs(tz_origin - tz_dest))
podatki
