...
Za razumevanje regularnih izrazov in tudi avtomatsko razlago priporočamo stran https://regex101.com/. Lahko si tudi ogledate knjigo Mastering Regular Expressions [@friedl2006mastering], ki obsežno obravnava snov regularnih izrazov.


## Domača naloga

1) Odprite datoteko DS-jobs.csv ter spremenite spremenljivko FormalEducation v tip faktor. Nato obdržite le štiri najbolj pogoste izobrazbe, ostale pa zamenjajte z vrednostjo "other". Uporabite eno izmed funkcij v skupini `fct_lump`.

```{r, echo = FALSE, message = FALSE}
ds_jobs <- read_csv2("./data-raw/DS-jobs.csv")
ds_jobs %>% mutate(FormalEducation = as.factor(FormalEducation)) %>%
  mutate(FormalEducation = fct_lump_n(FormalEducation, 4)) %>%
  select(Gender, Country, Age, FormalEducation)
```
  
2) Na podatkih `fruit` uporabite `str_subset` in poiščite:
    a) Vse sadeže, ki se začnejo in končajo na samoglasnik.
    ```{r, echo = FALSE, message = FALSE}
  str_subset(fruit, "^[aeiou].*[aeiou]$")
```

    b) Vse sadeže, ki imajo največ 5 črk.
    ```{r, echo = FALSE, message = FALSE}
  str_subset(fruit, "^.{1,5}$")
```

    c) **težja** Vse sadeže, pri katerih je prvi samoglasnik enak zadnjemu samoglasniku v besedi. Za rešitev je potrebno uporabiti negacijo in skupino.
  
    ```{r, echo = FALSE, message = FALSE}
  str_subset(fruit, "^[^aeiou]*([aeiou]).*(\\1)[^aeiou]*$")
```

3) Odprite prvi list ocen v `student-performance.xlsx` v mapi `test-script/data-raw/`.
...
shrani <- ocene %>% mutate(točke = averageScore/max(averageScore)*100,
                 ocena = ceiling(točke/10),
                 besedna_ocena = cut(ocena, breaks = c(-Inf, 5, 6, 7, 8, 9, 10),
                                     labels = c("nezadostno", "zadostno",
                                                "v redu", "dobro", 
                                                "zelo dobro", "odlično")))
shrani
```
    - Shranite podatke v datoteko `students-performance-math-normalized.xlsx' in v binarno datoteko `students-performance-math-normalized.rds'.
    - Ponovno preberite obe datoteki, prvo v spremenljivko `podatki_excel` in drugo v `podatki_binarni`. Prvo tudi spremenit v tibble. Katero razliko opazite?


    ```{r, echo = FALSE, message = FALSE}
write.xlsx(shrani, "./test-script/data-raw/students-performance-math-normalized.xlsx")
saveRDS(shrani, "./test-script/data-raw/students-performance-math-normalized.rds")
podatki_excel   <- as_tibble(read.xlsx("./test-script/data-raw/students-performance-math-normalized.xlsx"))
podatki_binarno <- readRDS("./test-script/data-raw/students-performance-math-normalized.rds")
podatki_excel
podatki_binarno
```

4) Naložite paket `nycflights13` s podatki letov. V tej bazi je več razpredelnic in sicer **flights**, **airlines**, **planes**, **airports** in **weather**. Za razumevanje si pomagajte z `?flights`.
...
```
    ...
    c) Za vsak let sestavite besedno poročilo spodnje oblike, ki ga shranite v stolpec *Poročilo leta*.
...
podatki <- left_join(flights, airports, by = c("origin" = "faa")) %>%
  mutate(name_origin = name, tzone_origin = tzone) %>%
  select(year, month, day, sched_dep_time,
         origin, dest, dep_delay, arr_delay, name_origin, tzone_origin)
podatki <- left_join(podatki, airports, by = c("dest" = "faa")) %>%
  mutate(name_dest = name, tzone_dest = tzone) %>%
  select(year, month, day, sched_dep_time,
         origin, dest, dep_delay, arr_delay, name_origin, tzone_origin,
         name_dest, tzone_dest)
podatki %>% mutate(dep_string = make_datetime(year, month, day,
...
```
    ...
    d) Časi odhodov in prihodov so v razpredelnici flights v lokalnem času. Dodajte spremenljivko `dep_time_utc`, ki bo vsebovala vrednost odhoda v časovnem pasu UTC. Zapis naj bo oblike integer, tako kot `dep_time'.

    ```{r, echo = FALSE, message = FALSE}
podatki <- left_join(flights, airports, by = c("origin" = "faa")) %>%
      select(year, month, day, dep_time,
         origin, dest, tzone) %>%
      mutate(dep_time_utc = make_datetime(year, month, day,
                                           hour = dep_time %/% 100,
                                           min = dep_time %% 100,
                                           tz = tzone),
             .after = dep_time) %>%
      mutate(dep_time_utc = with_tz(dep_time_utc, tz = "UTC")) %>%
      mutate(dep_time_utc = format(dep_time_utc, "%H%M"))
podatki

```

    e) Simuliramo. Vsi leti bodo zaradi nepričakovane prenove letališča po 23. 10. 2013 zamaknjeni za dva tedna. Izpišite posodobljene datume v originalni obliki (year, month, day).

    ```{r, echo = FALSE, message = FALSE}
flights %>% mutate(temp_d = make_date(year, month, day) + 2*weeks(), .after = day) %>%
  filter(temp_d >= make_date(2013, 10, 23) + 2*weeks()) %>%
  mutate(year = as.integer(year(temp_d)),
         month = as.integer(month(temp_d)),
         day = as.integer(day(temp_d))) %>%
  select(-temp_d)
```
